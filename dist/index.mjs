...
import { add as add2, isBefore, parseISO } from "date-fns";
import { nanoid as nanoid5 } from "nanoid";

// src/store/simulationStore.ts
import { create } from "zustand";
var useSimulationStore = create()((set, get) => ({
  // 初始状态
  contracts: [],
  shanghaiIndex: {
    current_price: 3500,
...
var FundContractEngine = class {
  ...
  generateHistoricalContracts() {
    ...
    while (isBefore(current, end)) {
      ...
      current = add2(current, { minutes: 5 });
    }
  }
  generateContract(issueTime, type) {
    ...
    const expiration = add2(now, { minutes: duration });
...
var exportToCSV = (data, filename) => {
  ...
  saveAs(dataBlob, `${filename}_${(/* @__PURE__ */ new Date()).toISOString().slice(0, 10)}.xlsx`);
};
